...
class ApplicationMain {
	...
	public static function main () {
		...
		//nme.Lib.setPackage("MaKu", "Hext", "hext.Hext", "1.0.0");
	...
	private static function start ():Void {
		...
		var hasMain = false;
		var mainClass = Type.resolveClass ("hext.Main");
...
class ApplicationMain {
	...
	public static function main () {
		...
		for (methodName in Type.getClassFields (hext.Main)) {
		...
		if (hasMain) {
			...
			Reflect.callMethod (hext.Main, Reflect.field (hext.Main, "main"), []);
...
@:keep class DocumentClass extends hext.Main { }
